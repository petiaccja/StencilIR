...
mlir::Operation* Converter::operator()(std::shared_ptr<OperationImpl> operation) {
    ...
    std::ranges::transform(operands, std::back_inserter(convertedOperands), [this](const auto& operand) {
        auto valueIt = m_convertedResults.find(operand->Result());
...
void Converter::MapEntryBlock(const RegionImpl& region, mlir::Block& block) {
